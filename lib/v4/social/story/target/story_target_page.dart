...
class _AmityStoryTargetSelectionPageState extends State<AmityStoryTargetSelectionPage> {
  ...
  void _schedulePermissionRecheck(int creatableCount, int totalCount) {
    if (creatableCount >= totalCount) return;
    if (_permissionRecheckCount >= _maxPermissionRecheckCount) return;
    if (_permissionRecheckTimer?.isActive ?? false) return;
    _permissionRecheckCount++;
    _permissionRecheckTimer =
      ...
      child: Scaffold(
        ...
        body: Consumer<MyCommunityVM>(
          builder: (context, viewModel, child) {
            final creatableCommunities = viewModel.amityCommunities
                .where(canCreateStoryInCommunityObject)
                .toList();
            _schedulePermissionRecheck(
                creatableCommunities.length, viewModel.amityCommunities.length);
