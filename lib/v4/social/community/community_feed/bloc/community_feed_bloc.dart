...
class CommunityFeedBloc extends Bloc<CommunityFeedEvent, CommunityFeedState> {
  ...
  CommunityFeedBloc({required this.communityId, required this.scrollController})
    ...
    communityStream.listen((community) {
      if(!isClosed){
        add(CommunityFeedEventJoinedStateUpdated(isJoined: community.isJoined ?? true));
      }
    });
    ...
    feedLiveCollection.getStreamController().stream.listen((posts) {
      if (!isClosed) {
        add(CommunityFeedEventPostUpdated(posts: posts));
      }
    });

    feedLiveCollection.observeLoadingState().listen((isLoading) {
      if (!isClosed) {
        add(CommunityFeedEventLoadingStateUpdated(isLoading: isLoading));
      }
    });

    announcementLiveCollection.getStreamController().stream.listen((announcements) {
      if (!isClosed) {
        add(CommunityFeedEventAnnouncementUpdated(announcements: announcements));
      }
    });

    pinLiveCollection.getStreamController().stream.listen((pins) {
      if (!isClosed) {
        add(CommunityFeedEventPinUpdated(pins: pins));
      }
    });
