...
class AmityStoryCommunityTabComponent extends NewBaseComponent {
  ...
  String? pageId;
  AmityStoryCommunityTabComponent({super.key, required this.communityId, this.pageId})
    : super(pageId: pageId, componentId: "story_tab_component");

  @override
  Widget buildComponent(BuildContext context) {
    return AmityStoryCommunityTabBuilder(theme: theme, communityId: communityId);
  }
}
...
class AmityStoryCommunityTabBuilder extends StatefulWidget {
  ...
  const AmityStoryCommunityTabBuilder({super.key, required this.theme, required this.communityId});
...
class _AmityStoryCommunityTabBuilderState extends State<AmityStoryCommunityTabBuilder> {
  ...
  void _refreshGlobalStoryTargets() {
    ...
    final tempCollection = GlobalStoryTargetLiveCollection(queryOption: AmityGlobalStoryTargetsQueryOption.SMART);
    ...
    return BlocListener<CreateStoryBloc, CreateStoryState>(
      listener: (context, state) {
        if (state is CreateStorySuccess) {
          context.read<AmityToastBloc>().add(
            const AmityToastShort(message: "Successfully shared story", icon: AmityToastIcon.success),
          );
        }
      },
      child: BlocBuilder<CommunityFeedStoryBloc, CommunityFeedStoryState>(
        builder: (context, state) {
          ...
          if (!_hasRefreshedOnEmpty && !state.isLoading && (state.stories == null || state.stories!.isEmpty)) {
          ...
          if (_hasRefreshedOnEmpty && state.stories != null && state.stories!.isNotEmpty) {
                ...
                children: [
                  ...
                  Shimmer.fromColors(
                    ...
                    child: Container(
                      width: 80,
                      height: 10,
                      decoration: BoxDecoration(color: widget.theme.baseColor, borderRadius: BorderRadius.circular(10)),
                    ),
                  ),
                ],
              ),
          ...
          if (state.community != null) {
            ...
            return Container(
              ...
              child: AmityStoryTargetElement(
                avatarUrl: state.community!.avatarImage?.getUrl(AmityImageSize.SMALL) ?? "",
                ...
                onClick: (targetId, storyTarget) {
                  if (state.haveStoryPermission &&
                      ...
                      (state.stories == null || state.stories?.isEmpty == true)) {
                    ...
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (BuildContext context) {
                          return AmityViewStoryPage(type: AmityViewStoryCommunityFeed(communityId: widget.communityId));
                        },
                      ),
                    );
                  }
