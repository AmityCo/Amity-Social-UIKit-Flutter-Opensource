...
class VideoPostPlayerBloc
    extends Bloc<VideoPostPlayerEvent, VideoPostPlayerState> {
  VideoPostPlayerBloc(
    ...
    on<VideoPostPlayerEventInitial>((event, emit) async {
      ...
      for (var post in posts) {
        final AmityVideo video =
            await (post.data as VideoData).getVideo(AmityVideoQuality.LOW);
        final videoUrl = video.getFileProperties!.fileUrl ?? "";
        urls.add(videoUrl);
        final String thumbnail =
            (post.data as VideoData).thumbnail?.fileUrl ?? "";
