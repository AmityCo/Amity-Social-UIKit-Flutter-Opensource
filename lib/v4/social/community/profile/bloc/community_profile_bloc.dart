    ...
    scrollController: scrollController,
        ...
        on<CommunityProfileEventUpdated>((event, emit) async {
          if (event.community != null) {
            ...
            .check() ?? false;
            final canManageStory = AmityCoreClient.hasPermission(AmityPermission.MANAGE_COMMUNITY_STORY).atCommunity(event.community.communityId!).check() ?? false;
            emit(state.copyWith(community: event.community, isJoined: event.community.isJoined, isModerator: isModerator, canManageStory: canManageStory));
          }
        });
