...
part 'group_banned_users_state.dart';
part 'group_banned_users_cubit.dart';

class GroupBannedUsersPage extends NewBasePage {
  final AmityChannel channel;

  GroupBannedUsersPage({
    Key? key,
    required this.channel,
  }) : super(key: key, pageId: 'group_banned_users_page');
  ...
  Widget buildPage(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) {
        final cubit = GroupBannedUsersCubit(
          channel: channel, 
      ...
      child: Builder(builder: (context) {
        ...
        return BlocBuilder<GroupBannedUsersCubit, GroupBannedUsersState>(
  ...
  Widget _buildBannedUsersList(BuildContext context, GroupBannedUsersState state) {
    if (state.isLoading) {
  ...
  Widget _buildBannedUsersListView(
      BuildContext context, GroupBannedUsersState state) {
    ...
    return userList(
      ...
      memberRoles: null,
      loadMore: () {
        context.read<GroupBannedUsersCubit>().loadMoreBannedUsers();
      },
      onTap: (user) {
  ...
  void _showBannedUserActions(BuildContext context, AmityUser user) {
    final state = context.read<GroupBannedUsersCubit>().state;
    final cubit = context.read<GroupBannedUsersCubit>();
