...
import '../bloc/add_group_member_cubit.dart';

class AddGroupMemberPage extends NewBasePage {
  AddGroupMemberPage({
    Key? key,
  ...
  Widget buildPage(BuildContext context) {
    return BlocProvider(
      create: (context) {
        final cubit = AddGroupMemberCubit();
        return cubit;
      },
      child: Builder(builder: (context) {
        // Initialize user list
        context.read<AddGroupMemberCubit>().queryUser('');

        return BlocBuilder<AddGroupMemberCubit, AddGroupMemberState>(
                ...
                child: Column(
                  children: [
                    AmityTopSearchBarComponent(
                      ...
                      onTextChanged: (value) {
                        _debouncer.run(() {
                          context.read<AddGroupMemberCubit>().queryUser(value);
                        });
                      },
  ...
  Widget userContainer(BuildContext context, AddGroupMemberState state) {
    if (state.isFetching == true) {
      ...
      return Container();
    } else if (state is AddGroupMemberState) {
      if (state.users.isEmpty) {
        ...
        return multiSelectUserList(
          ...
          loadMore: () {
            ...
            context.read<AddGroupMemberCubit>().loadmoreUsers();
          },
          onSelectUser: (user) {
            context.read<AddGroupMemberCubit>().updateSelectedUsers(user);
          },
